...
static inline int block_is_too_large(tFile *F, DWORD Size)
{
  ...
  LOG_PRINTF(("Initial occupation : %lu\n", InitialOccupation));
  LOG_PRINTF(("Final occupation   : %lu\n", FinalOccupation));
  LOG_PRINTF(("Free clusters      : %lu\n", F->V->FSI_Free_Count));
...
static int truncate_or_extend(tFile *F)
{
  ...
  else
  {
    ...
    LOG_PRINTF(("Unlinking clusters beyond the truncation position\n"));
    switch (F->V->FatType)
    {
      case FAT12:
