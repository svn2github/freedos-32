...
static int set_opening_mode(tFile *F, DWORD Mode)
{
  ...
  for (k = 0; k < NumFiles; k++)
  {
    ...
    if ((F != F1) && SAMEFILE(F1, F))
    {
      ...
      {
        if (FShare == 0) return -EINVAL; /* Sharing violation (INT 24h) */
                         return -EINVAL;
      }
    } /* if ((F != F1) && SAMEFILE(F1, F)) */
  }
...
static int descend_path(tVolume *V, char *Path, tFile **Fp)
{
  ...
  if ((F = take_file()) == NULL) return -ENFILE;
  F->V    = V;
  F->Mode = 0;
             ...
             WORD AliasHint, tFile **F)
{
  ...
  if (*FileName) for (Res = 0; Res < V->NumFiles; Res++)
    if (utf8_stricmp(FileName, V->Files[Res].CacheFileName) == 0)
    {
      ...
      if (Ff->References)
      {
        if ((Ff = get_fd(V)) == NULL) return -ENFILE;
  ...
  if (Res < 0)
  {
    ...
    /* Otherwise create the file */
     ...
     #endif
     *F = Ff;
     LOG_PRINTF(("FAT: File successfully created.\n"));
     return FD32_ORCREAT;
    #else
     Fp->References = 0;
     return -EROFS;
  ...
  *F = Ff;
  LOG_PRINTF(("FAT: File successfully opened.\n"));
  return FD32_OROPEN;
}
...
{
  if (F->References == 0) return -EBADF;
  #ifdef FATWRITE
  if (F->References - 1 == 0)
  {
    int Res;
  ...
  #endif
  LOG_PRINTF(("File closed. Volume buffers: %lu hits, %lu misses on %lu\n",
