/****************************************************************************/
/* FD32 FAT Driver test program "mkdir.c" by Salvo Isaja                    */
/* This program creates a new empty subdirectory.                           */
...
#define DEVNAME "hda1"
/* Set it to the canonicalized name of the directory to create */
#define DIRNAME "\\tmp\\newdir"
...
int fat_mkdirtest_init(void)
{
  ...
  Md.DeviceId = M.FsDev; /* Returned by the FD32_MOUNT request */
  Md.DirName  = DIRNAME;
  if ((Res = fat_request(FD32_MKDIR, &Md)) < 0)
  {
    message("Error %08xh while creating the directory '%s'\n", Res, DIRNAME);
    return Res;
  }
