...
void MMapWrite(unsigned long mmapsec, unsigned long * mmapbuf)
{
	dosmemput(mmapbuf, 512, sector_buffer);
	#ifdef __FORMAT_DEBUG__
	printf(" [map %lu] ", mmapsec);
	#endif
	if (Drive_IO(WRITE, mmapsec, -1) != 0)
		printf("Error writing MIRROR MAP sector %lu - UNFORMAT spoiled!\n",
		       mmapsec);
}
...
void Save_File_System(int overwrite)
{
	...
	unsigned long last_used_sector = 0;	/* BadClustPreserve returns this */
...
#define MAXCHUNKSIZE 16	/* could use one huge_sector_buffer completely */
	...
	char *fat_name;
	unsigned long bslong;
	unsigned short bsword;
#define BYTESIZE	(sizeof(unsigned char))
...
#define MMapCheckWrite if (pointer == (512/4)) { \
	...
	dosmemget(sector_buffer+510, WORDSIZE, &bsword);
	if ( bsword == 0xaa55 )
	{
		bad_boot_sector = FALSE;
		dosmemget(sector_buffer+0x0b, WORDSIZE, &bsword);
		if ( bsword != 512 )
		{
		...
		dosmemget(sector_buffer+0x10, BYTESIZE, &number_of_fats);
		...
		dosmemget(sector_buffer+0x0e, WORDSIZE, &reserved_sectors);
		if ((param.fat_type != FAT32) && (reserved_sectors != 1))
		{
			printf("WARNING: Number of reserved / boot sectors is %lu, not 1.\n",
			       reserved_sectors);
		...
		{
			printf("WARNING: Reserved sectors are %lu but will be %u after format.\n",
			       reserved_sectors, parameter_block.bpb.reserved_sectors);
			if ((reserved_sectors < 1) || (reserved_sectors > 64));
			bad_boot_sector = TRUE;
		}

		dosmemget(sector_buffer+0x11, WORDSIZE, &number_of_root_directory_entries);
		dosmemget(sector_buffer+0x16, WORDSIZE, &bsword);
		if ( ( (number_of_root_directory_entries == 0) ||
		        (bsword == 0) ) /* FAT1x size zero? */
		        && (fat_type != FAT32))
		{
			printf(" Confusing: FAT32 detected. Thought it would be %s!\n",
...
nomirror_wimp: /* common bailout point avoids duplicated code (0.91p) */
		...
		dosmemget(sector_buffer+0x13, WORDSIZE, &number_of_logical_sectors_on_drive);
		if (number_of_logical_sectors_on_drive==0)
			dosmemget(sector_buffer+0x20, LONGSIZE, &number_of_logical_sectors_on_drive);	/* if it was 0, use 32bit value */

		dosmemget(sector_buffer+0x16, WORDSIZE, &sectors_per_fat);
		if (sectors_per_fat == 0)
		{ /* FAT32 case: 32bit sectors per fat value */
			dosmemget(sector_buffer+0x24, LONGSIZE, &sectors_per_fat);
			if (fat_type != FAT32)
			{
				printf(" This must be FAT32, it has a zero FAT1x size.\n");
				goto nomirror_wimp;	/* 0.91p */
			}
		}
		else
		{ /* FAT1x case: 16bit sectors per fat value nonzero */
			if (number_of_root_directory_entries == 0)
			{
				printf(" This has a FAT32 type root directory but a FAT1x type FAT size!\n");
				goto nomirror_wimp;	/* 0.91p */
			}
			if (fat_type != ( (sectors_per_fat <= 12) ? FAT12 : FAT16 ))
			{
				printf(" Would be %s by FAT size rule but is supposed to be FAT32!\n",
		...
		{ /* not plausible root directory or FAT or drive size */
			printf(" Root directory or FAT or drive size not plausible! Bad boot sector?\n");
			...
			{
				printf(" UNFORMAT info would take more than half the space? Bad boot sector?\n");
				bad_boot_sector = TRUE;
			}
		} /* root directory an FAT and drive size was plausible */
		dosmemget(sector_buffer+0x0d, BYTESIZE, &sectors_per_cluster); /* only a byte */

		if (fat_type == FAT32)
		{
			beginning_of_root_directory = reserved_sectors + ( number_of_fats * (unsigned long)sectors_per_fat );
			...
			dosmemget(sector_buffer+0x2c, LONGSIZE, &bslong);
			if (bslong != 2)
	...
	else
	{
		...
		fat_name = "FAT1x";
		dosmemget(sector_buffer+0x16, WORDSIZE, &bslong);
		if(bsword == 0) {
			fat_name = "FAT32";
			dosmemget(sector_buffer+0x16, LONGSIZE, &bslong);
		}
		printf("FAT sizes are: FOUND %lu %s / PLANNED %lu %s\n",
			bslong, fat_name, scratch, (param.fat_type == FAT32) ? "FAT32" : "FAT1x");
	...
	mirror_size = reserved_sectors + sectors_per_fat + (1 + end_of_root_directory - beginning_of_root_directory);
	...
	mirror_beginning = ((number_of_logical_sectors_on_drive-1) - mirror_size) - offset_from_end; /* -1 because last=num-1, correct? */
	...
	dosmemput(&mirror_map_beginning, LONGSIZE, sector_buffer+0x0);
	dosmemput("AMSESLIFVASRORIMESAEP", 21, sector_buffer+4);
	...
	if (Drive_IO(WRITE, ((number_of_logical_sectors_on_drive-1) - offset_from_end), -1) != 0) /* -1 correct??? */
	{
	...
	do
	{
		if ( (source_sector>=reserved_sectors) /* "> 0" before 0.91p */ && (source_sector<beginning_of_fat) )
			...
			if (source_sector < beginning_of_fat) /* (some) BOOT sector */
			{
				if (source_sector == 0) /* better messages (0.91p) */
				{
					...
					if (reserved_sectors > 1)
						printf("[DEBUG]  (Messages for other RESERVED sectors skipped...)\n");
				}
				if ( (reserved_sectors > 1) &&
		...
		} while ( (chunksize < HUGE_SECTOR_BUFFER_SNUM) &&
		          (source_sector >= beginning_of_fat) &&
		          (end_of_root_directory_flag != TRUE) );
...
void Restore_File_System(void)
{
	...
	char number_of_fats = 0; /* unsigned CHAR this time! */
	...
	{
		char sz[24];
		dosmemget(sector_buffer+4, 21, sz);
		if (strncmp(sz, "AMSESLIFVASRORIMESAEP", 21))
		{
			printf("No MIRROR / UNDELETE data found: Wrong pointer magic.\n");
			return;
		}
	}
	dosmemget(sector_buffer+0x0, LONGSIZE, &mirror_map_sector);	/* read sector number of mirror map */
	pointer = 84/4;	/* start reading after headers! */

	Drive_IO(READ, mirror_map_sector, 1);
	dosmemget(sector_buffer+0, 512, mirror_map);

	dosmemput("", BYTESIZE, sector_buffer+13);	/* force string termination */
	#ifdef __FORMAT_DEBUG__
	printf("[DEBUG]  Mirror map at sector %lu, description '%s'\n",
		mirror_map_sector, (unsigned char *)mirror_map);
	#endif
	/* mirror map should start with string "x:\mirror.fil" */

	#ifdef __FORMAT_DEBUG__
	printf("[DEBUG]  First mirrored sector is %lu stored at %lu\n",
	...
	while (1)
	{
		...
		{
			printf("\n End marker of mirror map encountered. UNFORMAT done.\n");
			if (param.fat_type == FAT32)
			{
				Drive_IO(READ, 0, 1);
				dosmemget(sector_buffer+0x32, WORDSIZE, &backup_sector);
				...
				dosmemget(sector_buffer+0x30, WORDSIZE, &original_sector);
				        ...
				        && ((original_sector+backup_sector) < beginning_of_fat) )
				{
					...
					dosmemput("\xff\xff\xff\xff\xff\xff\xff\xff", 8, sector_buffer+488);
		...
		{
			unsigned char b;
			dosmemget(sector_buffer+0x10, BYTESIZE, &b);
			if ( (original_sector == 0) && ( (b > 7) || (b < 1) ) )
			{
				printf("Boot sector would be overwritten by nonsense!\n");
				printf("Aborting to avoid damage (0 or more than 7 FATs).\n");
			...
			{
				dosmemget(sector_buffer+0x10, BYTESIZE, &number_of_fats); /* should be or 1 2 */
				if ((number_of_fats < 0) || (number_of_fats > 2))
				{
					char b = number_of_fats;
				...
				dosmemget(sector_buffer+0x0e, WORDSIZE, &beginning_of_fat);	/* FAT begins after reserved sectors */
				dosmemget(sector_buffer+0x16, WORDSIZE, &sectors_per_fat);		/* FAT16 style */
				if (!sectors_per_fat)
				{
					dosmemget(sector_buffer+0x24, LONGSIZE, &sectors_per_fat);	/* FAT32 style */
					if (param.fat_type != FAT32)
