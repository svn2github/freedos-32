...
{
	printf("Unusable media: Bad sector in metadata area. Format terminated.\n");
	Exit(4,30);
} /* unusable_wimp */
...
void Record_Bad_Clusters(void)
{
	if (parameter_block.bpb.sectors_per_cluster == 0)
	{
		...
		printf("\nPlease report this BUG: 0 sectors / cluster in Record_Bad_Clusters!\n");
		Exit(4,31);
	}
...
void Record_Bad_Clusters_FAT12(void)
{
	...
	for (sector = 0; sector < parameter_block.bpb.sectors_per_fat; sector++)
	{
		...
		dosmemput(fat12_fat+fat_index, 512, sector_buffer);
...
void Record_Bad_Clusters_FAT16(void)
{
	...
	do
	{
		...
		do
		{
			...
			{
				dosmemput("\xf7\xff", 2, sector_buffer+2*(bad_cluster&0x00ff));
		...
		if (dirty_sector_buffer==TRUE) /* if anything inside, write it */
		{
			if (current_fat_sector==1)
			{
				...
				dosmemput("\xff\xff\xff", 3, sector_buffer+1);
			}
...
void Record_Bad_Clusters_FAT32(void) /* added actual FAT writing in 0.91j */
{
	...
	while ( (bad_sector_map[bad_sector_map_index] != 0) )
	{
		...
		dosmemget(sector_buffer+fat_index, 4, &fat_value);
		...
		if ( (fat_value < 0x0ffffff0UL) || (fat_value > 0x0ffffff7UL) ) /* not yet marked? */
		{
			...
			dosmemput("\xf0\xff\xff\x0f", 4, sector_buffer+fat_index);
			/* sector_buffer+fat_index+0: mark cluster as bad */
			/* sector_buffer+fat_index+2: range for the "bad" mark: */
			/* sector_buffer+fat_index+3: 0x0ffffff0 ... 0x0ffffff7 */
